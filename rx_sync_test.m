...
%     radio.DecimationFactor = 100;    
    ...
    % % Capture Signal
    burstCaptures = zeros(19200,2,NumFramesInBurst);
    len = 0;
...
tagWave = reshape(burstCaptures(:,1,:),[],1);
enbWave = reshape(burstCaptures(:,2,:),[],1);
Global_Parameters;
h128s = zeros(128,4,80);
%% OFFSET
for offset = 21:30  %%80个5ms  11-90
    rxWaveform = tagWave(19200*offset:19200*offset+19200*2);
    rxWaveformCorrected = waveformCorrect(rxWaveform,enb);
    rxWaveformEnb = enbWave(19200*offset:19200*offset+19200*2);
    rxWaveformEnbCorrected = waveformCorrect(rxWaveformEnb,enb);
    rxWaveformEnbRecovered = EnbRecover(EnbRecover,)
    %% 接收机解调TAG
    tag_signal_1pss = rxWaveformCorrected(961:9600-960);%%%%只取前5ms
...
%         figure
%         plot(abs(H128_est))
 %          load("H128_est_test.mat");
%          H128_est= H128_est_test;
%          h128s(:,index+1,offset-10)=H128_est;
          ps(index+1,offset-10)=p_est;
