...
%#codegen
%产生1个子帧（1200子载波 14个OFDM符号  1ms）  根据子帧序号不同0/5/其他，分配资源格
% prmLTE=prmLTEPDSCH;  %测试本函数用
switch nargin
%     case 4, pdcch=[];pss=(-0.707 +0.707j).*ones(1,72)*2;sss=[];bch=[];pss=[];
...
idx_dci=1:ContREs;
idx_pdcch = ExpungeFrom(idx_dci,idx_csr0); %1000
...
%% Handle 3 types of subframes differently  不同子帧不一样
switch nS
    ...
    case 0    % Subframe 0
        ...
        idx_6rbs = (1:72);%     频域上6个RB所占的子载波 72个子载波
        idx_ctr = 0.5* lenOFDM - 36 + idx_6rbs ;
        idx_SSS  = 5* lenOFDM + idx_ctr;
        idx_PSS  = 6* lenOFDM + idx_ctr;
        idx_ctr0 = ExpungeFrom(idx_ctr,idx_csr0);
        idx_bch=[7*lenOFDM + idx_ctr0, 8*lenOFDM + idx_ctr, 9*lenOFDM + idx_ctr, 10*lenOFDM + idx_ctr];%276
