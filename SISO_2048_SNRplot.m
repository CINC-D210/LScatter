...
snrdB_v            = 0:2:30;   % Value of SNR used in this experiment
ber_vec=zeros(1,length(snrdB_v))
for cnt_snr=1:length(snrdB_v);
    snrdB=snrdB_v(cnt_snr);
...
[txSig_tag,p,offset_phi,preamble_p,BIT]=Modbytag(txSig);  %p为 offset_int 
%%  2 Channel
% SISO Fading channel
...
Bit10(find(BIT==1))=0;
Bit10(find(BIT==-1))=1;
[number,ratio,loc] = symerr(bit_seq_matrix,Bit10(:,1:12));

ber_vec(cnt_snr)=ratio;
end
...
grid on
set(gca,'yscale','log');
plot(snrdB_v,ber_vec,'b-o','LineWidth',2);
xlabel("Eb/N0 dB)")
ylabel("BER of BD ")
