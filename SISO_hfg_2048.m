...
snrdB            = 30;   % Value of SNR used in this experiment

%tag信道
prmMdl_Tag_f.chanMdl = chanMdl;
prmMdl_Tag_f.corrLevel = 'frequency-selective-low-mobility';
prmMdl_Tag_f.chEstOn = 1;  %是否估计信道

prmMdl_Tag_g.chanMdl = chanMdl;
% prmMdl_Tag_g.corrLevel = 'frequency-selective-low-mobility';
prmMdl_Tag_g.corrLevel ='flat-low-mobility';
prmMdl_Tag_g.corrLevel = 'frequency-selective-low-mobility';
...
%%  2.2  Channel-f-tag-g 
% TX-f
[Sig_f, chPathG_tag_f] = MIMOFadingChan_Tag_f(txSig, prmLTEPDSCH, prmMdl_Tag_f);
% Sig_f=txSig*0.01;
%TAG调制（14个OFDM+CP）
[Sig_f_tag,p,offset_phi,preamble_p,BIT,Bit01]=Modbytag(Sig_f);  %p为 offset_int 

% TX-f-Tag-g
[rxFade_tag, chPathG_tag_g] = MIMOFadingChan_Tag_g(Sig_f_tag, prmLTEPDSCH, prmMdl_Tag_g);
% rxFade_tag=Sig_f_tag*1;
...
for cnt=3:2+12
bit_seq2=tagdecode2048_ifft(p_est, yRec_tag2048(:,cnt), txGrid(:,cnt));
